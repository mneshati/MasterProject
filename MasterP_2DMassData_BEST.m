...
Ustep=50;
%-----------------------------
m=100; %m=10 mass of the moving particle  
T=0.1; %T=0.001 sampling time 
K=100; %K=100000 (K*sampling time) is the time that results are read 
...
%% Quality calculation
            ...
            for i=1:8
                ...
                DistanceTo=sqrt((TempA^2)+(TempB^2)); %distance (transition quality) to neighbour(i) 
                FullData(CL,CV).DisToNei(i,Uc)=DistanceTo;%save each distance(quality)for the specific U 
            end
            Uc=Uc+1;
       end
       Uc=1;
...
for i=1:9
    for j=1:9
        for k=1:9
            for m=1:9
        DijFullData(i,j).Q(k,m)=inf;
        DijFullData(i,j).U(k,m)=inf;
    ...
    for CV0=1:9
        ...
        for i=1:8
            ...
            if (CL>0 & CV>0 & CL<10 & CV<10)
                ...
                %TableCoY=((CL-1)*9)+CV;
                DijFullData(CL0,CV0).Q(CL,CV)=MinDis;
                TempC=FindU(InputIndCausedMinDis);
                DijFullData(CL0,CV0).U(CL,CV)=TempC;
            end
        end
    end
end
